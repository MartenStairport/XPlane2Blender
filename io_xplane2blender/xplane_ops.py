...
class AutoLOD(bpy.types.Operator):
    """Automatically generate 4-levels LOD for the selected object
    IMPORTANT: make sure that the collection name on the scene is the same with the exporter collection name
    """
    bl_idname = "stairport_xplane_lod.auto_lod"
    bl_label = "Generate 4-levels LOD"
    bl_description = "Sets up the selected object and auto generates 4-stages LOD\nIMPORTANT: make sure that the collection name on the scene is the same with the exporter collection name"
    ...
    def object_setup(self, context):
        cur_obj = context.active_object
        base_name = cur_obj.name

        # LOD 1: modification done directly on the selected object
        print('processing LOD 1')
        cur_obj.name = base_name + '_LOD1'
        cur_obj.xplane.override_lods = True
        cur_obj.xplane.lod = (True, False, False, False)

        # LOD 2
        print('processing LOD 2')
        bpy.ops.object.duplicate()
        cur_obj = context.active_object
        cur_obj.name = base_name + '_LOD2'
        cur_obj.xplane.override_lods = True
        cur_obj.xplane.lod = (False, True, False, False)
        decimate = cur_obj.modifiers.new('Decimate', 'DECIMATE')
        decimate.decimate_type = 'COLLAPSE'
        decimate.ratio = 0.75

        # LOD 3
        print('processing LOD 3')
        bpy.ops.object.duplicate()
        cur_obj = context.active_object
        cur_obj.name = base_name + '_LOD3'
        cur_obj.xplane.override_lods = True
        cur_obj.xplane.lod = (False, False, True, False)
        cur_obj.modifiers['Decimate'].ratio = 0.5

        # LOD 4
        print('processing LOD 4')
        bpy.ops.object.duplicate()
        cur_obj = context.active_object
        cur_obj.name = base_name + '_LOD4'
        cur_obj.xplane.override_lods = True
        cur_obj.xplane.lod = (False, False, False, True)
        cur_obj.modifiers['Decimate'].ratio = 0.25

    def calculate_max_dist(self, obj_dimension, limit=20000):
        min_theta = self.fov * self.minObjDimension / self.screenResAlongDimension
        min_theta_radians = self.to_radians(min_theta)
        max_dist = obj_dimension / (2.0 * math.sin(min_theta_radians/2.0))
        if max_dist > limit:
            print('max dist out of range:', max_dist,
                  'will be overriden to limit:', limit)
            max_dist = limit
        return max_dist

    def calculate_lod_distances(self, max_dist):
        print('max dist', max_dist)
        distances = []
        distances.append(0.2 * max_dist)
        distances.append(0.35 * max_dist)
        distances.append(0.5 * (max_dist - distances[1])+(0.35 * max_dist))
        distances.append(max_dist)
        print('lod distances:', distances)
        return distances
    ...
    def execute(self, context):
        print('setting up object for LOD')
        self.object_setup(context)

        print('processing lod for collection:', self.collection_name)
        coll = bpy.data.collections[self.collection_name]
        coll.xplane.layer.lods = "4"
        num_lods = int(coll.xplane.layer.lods)
        if num_lods == 4:
            # get max dimension from currently active object
            dim = context.active_object.dimensions
            print('selected object dimensions', dim[0], dim[1], dim[2])
            max_obj_dim = max(dim)

            # calc lod distances
            max_dist = self.calculate_max_dist(max_obj_dim)
            lod_distances = self.calculate_lod_distances(max_dist)

            # apply
            for i, lod in enumerate(coll.xplane.layer.lod[:num_lods]):
                if i == 0:
                    lod.near = 0
                else:
                    lod.near = lod_distances[i-1]
                lod.far = lod_distances[i]

        else:
            print('lod count is not set to 4, aborting')

        return {'FINISHED'}


bpy.utils.register_class(AutoLOD)
